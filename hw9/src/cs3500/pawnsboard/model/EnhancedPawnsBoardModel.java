...
public class EnhancedPawnsBoardModel implements IPawnsBoardModel {
  ...
  /**
   * For testing only: apply the influence of `card` at (centerR, centerC)
   * without actually placing the card or changing turn state.
   */
  void rawApplyInfluence(int centerR, int centerC, Card card) {
    this.applyInfluence(centerR, centerC, card);
  }
  ...
  protected final PlayerState blueState;
  ...
  public final int[][] valueModifiers;
  ...
  /**
   * Give me the mutable hand list for the given player so demos/tests can seed cards.
   */
  public List<Card> getHandFor(PlayerColor who) {
    return (who == PlayerColor.RED)
            ? this.redState.getHand()
            : this.blueState.getHand();
  }
