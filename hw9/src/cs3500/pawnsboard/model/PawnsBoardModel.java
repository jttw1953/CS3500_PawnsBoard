...
public class PawnsBoardModel implements IPawnsBoardModel {
  ...
  /**
   * Register a new observer to be notified of model changes.
   */
  ...
  /**
   * Notify all observers that the current player changed.
   */
  ...
  /**
   * Notify all observers that the game ended, providing the winner (or null if tie).
   */
  ...
  public void pass() {
    ...
    if (lastWasPass) {
      ...
      switchPlayer();
      // switchPlayer() automatically notifies turn changed
    }
  }
  ...
  private void switchPlayer() {
    ...
    // Now that currentPlayer changed, notify observers
    notifyTurnChanged();
  }
  ...
  public boolean canPlaceCard(int handIndex, int row, int col) {
    ...
    return cell.getPawnCount() >= cardToPlace.getCost();
  }
