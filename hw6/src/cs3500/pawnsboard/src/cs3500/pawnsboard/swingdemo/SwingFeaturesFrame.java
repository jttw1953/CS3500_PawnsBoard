package cs3500.pawnsboard.swingdemo;
...
public class SwingFeaturesFrame extends JFrame implements ActionListener, ItemListener, ListSelectionListener {
  ...
  private JList<Integer> multiSelectListOfIntegers;
  ...
  public SwingFeaturesFrame() {
    ...
    JPanel multiSelectionListPanel = new JPanel();
    multiSelectionListPanel.setBorder(BorderFactory.createTitledBorder("Multi selection list"));
    multiSelectionListPanel.setLayout(new BoxLayout(multiSelectionListPanel, BoxLayout.X_AXIS));
    mainPanel.add(multiSelectionListPanel);

    JLabel instructionLabel = new JLabel("Right-click on elements to select more than one");
    multiSelectionListPanel.add(instructionLabel);
    
    dataForListOfIntegers = new DefaultListModel<>();
    for (int i = 0; i < 1000; i++)
      dataForListOfIntegers.addElement(i);
    multiSelectListOfIntegers = new JList<>(dataForListOfIntegers);
    multiSelectListOfIntegers.setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    multiSelectionListPanel.add(new JScrollPane(multiSelectListOfIntegers));
    
    JButton chosenButton = new JButton("Present numbers chosen");
    chosenButton.setActionCommand("Present numbers");
    chosenButton.addActionListener(this);
    multiSelectionListPanel.add(chosenButton);
    ...
    JMenuItem helloMenuItem = fileMenu.add("Say hi!");

    //MenuItems react to ActionEvents, like JButtons!
    helloMenuItem.setActionCommand("Hello");
    helloMenuItem.addActionListener(this);
    
    JMenuItem closeProgramItem = fileMenu.add("Close the program");

    closeProgramItem.setActionCommand("Goodbye");
  ...
  public void actionPerformed(ActionEvent arg0) {
    ...
    switch (arg0.getActionCommand()) {
      case "Hello":
	JOptionPane.showMessageDialog(SwingFeaturesFrame.this, "Hello out there!", "Say hi", JOptionPane.PLAIN_MESSAGE);
	break;
	...
	break;
      ...
      break;
    case "Present numbers":
	JFrame presentNumbersFrame = new PresentNumFrame(multiSelectListOfIntegers.getSelectedValuesList());
	presentNumbersFrame.setVisible(true);
	break;
    }
  }
