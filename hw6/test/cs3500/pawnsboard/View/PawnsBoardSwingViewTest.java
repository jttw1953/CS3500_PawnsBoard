    ...
    public void testBoardPanelMouseListener() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ReadOnlyPawnsBoardModel emptyModel = new ReadOnlyPawnsBoardModel() {
                @Override public int getRows() { return 2; }
                @Override public int getCols() { return 2; }
                @Override public PlayerColor getCurrentPlayer() { return PlayerColor.RED; }
                @Override public boolean isGameOver() { return false; }
                @Override public PlayerColor getWinner() { return null; }
                @Override public ReadOnlyCell getCellState(int row, int col) {
                    // Return an empty cell
                    return new ReadOnlyCell(CellType.EMPTY, null, 0, "");
                }
                @Override public List<Card> getCurrentPlayerHand() { return new ArrayList<>(); }
                @Override public int getRowScore(int row, PlayerColor color) { return 0; }
                @Override public int getTotalScore(PlayerColor color) { return 0; }
            };
