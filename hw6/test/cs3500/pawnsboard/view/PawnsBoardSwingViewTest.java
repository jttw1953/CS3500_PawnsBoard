...
public class PawnsBoardSwingViewTest {
  ...
  public void testConstructAndShow() throws Exception {
    SwingUtilities.invokeAndWait(() -> {
      ...
      IPawnsBoardModel model = new PawnsBoardModel(2, 3, new ArrayList<>(deck), new ArrayList<>(deck), 1);
      ...
      // ✅ Add assertions to satisfy JUnit rules
      assertEquals("Controller should be set correctly", controller, ((PawnsBoardSwingView) guiView).getController());
      assertTrue("Frame should be visible", ((PawnsBoardSwingView) guiView).isVisible());
    });
  }
  ...
  public void testBoardPanelMouseListener() throws Exception {
    SwingUtilities.invokeAndWait(() -> {
      ReadOnlyPawnsBoardModel emptyModel = new ReadOnlyPawnsBoardModel() {
        ...
        @Override public PlayerColor getCurrentPlayer() { 
      ...
      // ✅ Assert mouse listeners present
      assertEquals("Expected exactly 1 mouse listener", 1, board.getMouseListeners().length);
    });
  }
  ...
  public void testHandPanelMouseListener() throws Exception {
    SwingUtilities.invokeAndWait(() -> {
      ...
      // ✅ Assert mouse listeners present
      assertEquals("Expected exactly 1 mouse listener", 1, handPanel.getMouseListeners().length);
    });
  }
